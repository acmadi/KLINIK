...
Public Class FormMsJenisFishing
    ...
    Private Sub generateCode()
        ...
        Dim kode As String
        Dim reader = getCode("msjenisfishing", "KdJenisFishing")
    ...
    Private Sub viewAllData(ByVal cr As String)
        sql = "select KdJenisFishing,JenisFishing from  " & tab
        If cr <> "" Then
            sql = sql & "  where JenisFishing like '%" & cr & "%'"
        End If
    ...
    End Sub
    Private Sub FormMsJenisFishing_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        tab = " MsJenisFishing "
        PK = "  KdJenisFishing  "
        ubahAktif(False)
        viewAllData("")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If txtNoPolisi.Text <> "" Then
            If MsgBox("Apakah Anda yakin ingin menghapus tipe Fishing " & txtNoPolisi.Text & "?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmation") = MsgBoxResult.Yes Then
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If txtNoPolisi.Text = "" Then
            msgInfo("Tipe Fishing harus diisi")
            txtNoPolisi.Focus()
        Else
            '  Dim tg As String = String.Format("{0:yyyy-MM-dd}", txtTgl.Value)
            If status = "add" Then
                Try
                    Dim reader = execute_reader("Select * from " & tab & " where JenisFishing ='" & Trim(txtNoPolisi.Text) & "'")
                    If reader.Read Then
                        msgInfo("Tipe Fishing telah ada di dalam database")
                        txtNoPolisi.Text = ""
                        txtNoPolisi.Focus()
                        reader.Close()
                    Else
            ...
            ElseIf status = "update" Then
                Try
                    sql = "update   " & tab & "  set JenisFishing='" & Trim(txtNoPolisi.Text) & "'" & _
                    "  where  " & PK & "='" + txtID.Text + "' "
                    execute_update(sql)
                    viewAllData("")
                    ubahAktif(False)
