...
Public Class FormBrowseDistributor
    ...
    Private Sub viewAllData(ByVal cr As String, ByVal opt As String)
        sql = " SELECT KdDistributor 'Call ID', Nama 'Distributor', " & _
              " Alamat, Daerah, NoTelp 'No.Telepon', NoHP 'No. HP', " & _
              " noFax 'No. Fax' " & _
        ...
        sql &= " ORDER BY Nama "
        gridSupplier.DataSource = execute_datatable(sql)

        setGrid()
    End Sub

    Private Sub setGrid()
        With gridSupplier.ColumnHeadersDefaultCellStyle
    ...
    Function getSales()
        Try
            If gridSupplier.RowCount Then
                data_carier(0) = gridSupplier.CurrentRow.Cells("Call ID").Value
                data_carier(1) = gridSupplier.CurrentRow.Cells("Distributor").Value
            End If
    ...
    Private Sub gridSales_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridSupplier.DoubleClick
        getSales()
        Me.Close()
    End Sub
