...
Public Class FormMsJenisKardus
    ...
    Private Sub viewAllData(ByVal cr As String)
        sql = "select *  from  " & tab
        If cr <> "" Then
            sql = sql & "  where JenisKardus like '%" & cr & "%'"
        End If
    ...
    End Sub
    Private Sub FormMsJenisKardus_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        tab = " MsJenisKardus "
        PK = "  KdJenisKardus  "
        ubahAktif(False)
        viewAllData("")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If txtNoPolisi.Text <> "" Then
            If MsgBox("Apakah Anda yakin ingin menghapus jenis kardus " & txtNoPolisi.Text & "?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmation") = MsgBoxResult.Yes Then
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If txtNoPolisi.Text = "" Then
            msgInfo("Jenis kardus harus diisi")
            txtNoPolisi.Focus()
        Else
            '  Dim tg As String = String.Format("{0:yyyy-MM-dd}", txtTgl.Value)
            If status = "add" Then
                Try
                    Dim reader = execute_update("Select * from " & tab & " where jeniskardus ='" & Trim(txtNoPolisi.Text) & "'")
                    If reader.Read Then
                        msgInfo("Jenis kardus telah ada di dalam database")
                        txtNoPolisi.Text = ""
                        txtNoPolisi.Focus()
                        reader.Close()
                    Else
            ...
            ElseIf status = "update" Then
                Try
                    sql = "update   " & tab & "  set JenisKardus='" & Trim(txtNoPolisi.Text) & "'" & _
                    "  where  " & PK & "='" + txtID.Text + "' "
                    execute_update(sql)
                    viewAllData("")
                    ubahAktif(False)
