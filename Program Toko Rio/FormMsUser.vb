...
Public Class FormMsUser
    ...
    Private Sub setData()
        ...
        Try
            ...
            txtAlamat.Text = DataGridView1.CurrentRow.Cells("Alamat").Value.ToString
            txtkeahlian.Text = DataGridView1.CurrentRow.Cells("Keahlian").Value.ToString

            If DataGridView1.CurrentRow.Cells(3).Value.ToString = 1 Then
                temp = "1 - " & "Staff Apotek"
            ElseIf DataGridView1.CurrentRow.Cells(3).Value.ToString = 2 Then
                temp = "2 - " & "Staff Laboratorium"
    ...
    Private Sub viewAllData(ByVal cr As String)
        ...
        End If
        If Val(is_access) > 2 Then
            btnUpdate.Enabled = True
        End If
        sql = "select UserID 'User ID',Username,NamaLengkap 'Nama Lengkap',Level, Password,Alamat,Keahlian from  " & tab
        If cr <> "" Then
        ...
        Else
            ...
            If status = "add" Then
                Try
                    ...
                    Else
                        reader.Close()
                        sql = "insert into  " & tab & "  values('" + Trim(txtID.Text) + "','" & Trim(txtNoPolisi.Text) & "'," & _
                        "'" & Trim(txtNama.Text) & "','" & txtNamaL.Text & "','" & Trim(LevelID(0)) & "'," & _
                        "'" & Trim(txtAlamat.Text) & "','" & Trim(txtkeahlian.Text) & "')"
                        Try
                            execute_update(sql)
                            viewAllData("")
                            ubahAktif(False)
            ...
            ElseIf status = "update" Then
                Try
                    ...
                    " Alamat ='" & Trim(txtAlamat.Text) & "',Keahlian='" & Trim(txtkeahlian.Text) & "' " & _
                    "  where  " & PK & "='" + txtID.Text + "' "
                    execute_update(sql)
                    viewAllData("")
                    ubahAktif(False)
