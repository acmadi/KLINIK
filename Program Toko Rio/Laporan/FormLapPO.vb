...
Public Class FormLapPO
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select DATE_FORMAT(Tanggal_PO,'%d-%m-%Y') `Tgl PO`,c.Nama `Supplier`,mp.KdBarang `Part No.`,NamaBarang,Merk,Subkategori " & _
        ", jumlah `Qty`  " & _
        ", CASE WHEN StatusPO = 0 THEN 'New' WHEN StatusPO = 1 THEN 'Confirm' " & _
        " WHEN StatusPO = 2 THEN 'Barang Diterima' End 'Status PO' " & _
        " from trheaderpo ho join trdetailpo do  on ho.no_po=do.no_po join Msbarang mp on mp.KDBarang=do.KDBarang" & _
        "  join mssupplier c on c.kdsupplier = ho.kdsupplier  Join MsMerk On MsMerk.kdMerk = mp.kdMerk Join MsSubkategori On MsSubkategori.KdSub = mp.KdSub " & _
        "  where  DATE_FORMAT(Tanggal_PO,'%Y-%m-%d') >='" & tg1 & "' and DATE_FORMAT(Tanggal_PO,'%Y-%m-%d') <='" & tg2 & "'" & _
        "  and StatusPO <> 0 And TipePO = 0"
        ...
        Try
            tglMulai = tg1
            tglAkhir = tg2
            dropview("viewCetakLapPOUS11010001")
            createview(query, "viewCetakLapPOUS11010001")
