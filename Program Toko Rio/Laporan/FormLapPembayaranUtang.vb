...
Public Class FormLapPembayaranUtang
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select KdPurchasePayment `No Pembayaran`,DATE_FORMAT(TanggalPurchasePayment,'%d/%m/%Y') `Tgl Pembayaran`,Nama `Nama Supplier`,No_PB `No Penerimaan Barang`,FORMAT(TotalPurchasePayment,0) `Total Pembayaran`  "
        query2 = " select KdPurchasePayment `No Pembayaran`,DATE_FORMAT(TanggalPurchasePayment,'%d/%m/%Y') `Tgl Pembayaran`,Nama `Nama Supplier`,No_PB `No Penerimaan Barang`,TotalPurchasePayment `Total Pembayaran`  "

        queryFrom += "  from  trpurchasepayment ho join mssupplier c on c.kdsupplier = ho.kdsupplier  "
