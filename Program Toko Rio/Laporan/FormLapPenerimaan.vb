...
Public Class FormLapPenerimaan
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select ho.no_po `No PO`,ho.no_Pb `No. Penerimaan`,DATE_FORMAT(Tanggal_TerimaBarang,'%d-%m-%Y') `Tgl Terima Barang`,c.Nama `Supplier`,mp.KdBarang `Part No.`,NamaBarang,Merk,Subkategori " & _
        ...
        "  from trheaderpb ho join trdetailpb do  on ho.no_pb=do.no_pb join Msbarang mp on mp.KDBarang=do.KDBarang" & _
        " join trheaderpo hpo on hpo.no_po=ho.no_po  join mssupplier c on c.kdsupplier = hpo.kdsupplier  Join MsMerk On MsMerk.kdMerk = mp.kdMerk Join MsSubkategori On MsSubkategori.KdSub = mp.KdSub " & _
       " left join(" & _
       ...
       " where StatusRetur <> 0  " & _
        ...
        "  and  StatusTerimaBarang <> 0 And TipePB = 0 " & _
        "  group by ho.no_po,ho.no_Pb,`Tgl Terima Barang`,c.Nama,do.KdBarang,NamaBarang,Merk,Subkategori,Qty,Harga "
