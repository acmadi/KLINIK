...
Public Class FormLapPenerimaanFising
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select ho.no_po `No PO`,ho.no_Pb `No. Penerimaan`,DATE_FORMAT(Tanggal_TerimaBarang,'%d-%m-%Y') `Tgl Terima Barang`,mp.KdFising `No Fising`,TypeFising,JenisFising " & _
        ", do.Qty,FORMAT((harga - (harga*disc/100)),0) Harga, FORMAT(sum(do.Qty*(harga - (harga*disc/100))) - ifnull(`Total Retur`,0) ,0) `Total`  " & _
        ", CASE WHEN StatusPaid = 1 THEN 'Lunas' " & _
        " WHEN StatusPaid = 0 THEN 'Belum Lunas' End 'Pembayaran'" & _
        "  from trheaderpb ho join trdetailpb do  on ho.no_pb=do.no_pb join msfising mp on mp.KdFising=do.KDBarang " & _
        " join trheaderpo hpo on hpo.no_po=ho.no_po  join mssupplier c on c.kdsupplier = hpo.kdsupplier   " & _
       " left join(" & _
       "      select ho.KdPB,ho.kdretur,  IFNULL(sum(qty * (harga - (harga*disc/100))),0)  `Total Retur` " & _
       ...
       " where StatusRetur <> 0  " & _
        ...
        "  and  StatusTerimaBarang <> 0 And TipePB = 2 " & _
        "  group by ho.no_po,ho.no_Pb,`Tgl Terima Barang`,c.Nama,mp.KdFising,TypeFising,JenisFising,do.Qty,Harga "
        ...
        Do While reader21.Read
            ...
            supplier(idAr) = ""
            Part_No(idAr) = reader21("No Fising")
            Nama_barang(idAr) = "" 'reader21("NamaBahanMentah")
            merk(idAr) = reader21("TypeFising")
            subkategori(idAr) = reader21("JenisFising")
        ...
        Dim x As Integer = 0
        For x = 0 To idAr - 1
            sql = "insert into v_total_pb_fising(NO_PO,NO_PB,Tgl_Terima_Barang,Supplier,Part_No,NamaBarang,Merk,Subkategori,Qty,Harga,Total,Pembayaran) " & _
    ...
    Private Sub createTabTotal()
        Try
            Dim sql As String
            sql = " create Table v_total_pb_fising (" & _
    ...
    Private Sub dropTableTotal()
        Try
            Dim sql As String
            sql = "drop table v_total_pb_fising "
            execute_update(sql)
        Catch ex As Exception
        End Try
    End Sub

    Private Sub FormLapPenerimaanFising_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
