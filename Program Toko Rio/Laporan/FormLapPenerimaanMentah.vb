...
Public Class FormLapPenerimaanMentah
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select ho.no_po `No PO`,ho.no_Pb `No. Penerimaan`,DATE_FORMAT(Tanggal_TerimaBarang,'%d-%m-%Y') `Tgl Terima Barang`,c.Nama `Supplier`,mp.KdBahanMentah `No Bahan Mentah`,NamaBahanMentah " & _
        ", Qty,FORMAT((harga - (harga*disc/100)),0) Harga, FORMAT(sum(Qty*(harga - (harga*disc/100))) - ifnull(`Total Retur`,0) ,0) `Total`  " & _
        ", CASE WHEN StatusPaid = 1 THEN 'Lunas' " & _
        " WHEN StatusPaid = 0 THEN 'Belum Lunas' End 'Pembayaran'" & _
        "  from trheaderpb ho join trdetailpb do  on ho.no_pb=do.no_pb join msbahanmentah mp on mp.KDBahanMentah=do.KDBarang " & _
       ...
       " where StatusRetur <> 0  " & _
        ...
        "  and  StatusTerimaBarang <> 0 And TipePB = 1 " & _
        "  group by ho.no_po,ho.no_Pb,`Tgl Terima Barang`,c.Nama,mp.KdBahanMentah,NamaBahanMentah,Qty,Harga "
        ...
        Do While reader21.Read
            ...
            Part_No(idAr) = reader21("No Bahan Mentah")
        ...
        Dim x As Integer = 0
        For x = 0 To idAr - 1
            sql = "insert into v_total_pb_mentah(NO_PO,NO_PB,Tgl_Terima_Barang,Supplier,Part_No,NamaBarang,Merk,Subkategori,Qty,Harga,Total,Pembayaran) " & _
        ...
        End Try
    End Sub
    Private Sub FormLapPenerimaanMentah_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub createTabTotal()
        Try
            Dim sql As String
            sql = " create Table v_total_pb_mentah (" & _
    ...
    Private Sub dropTableTotal()
        Try
            Dim sql As String
            sql = "drop table v_total_pb_mentah "
            execute_update(sql)
        Catch ex As Exception
        End Try
    End Sub
