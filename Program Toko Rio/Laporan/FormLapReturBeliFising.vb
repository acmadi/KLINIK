...
Public Class FormLapReturBeliFising
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select DATE_FORMAT(TanggalRetur,'%d-%m-%Y') `Tgl Retur`,mp.KdFising `No Fising`,TypeFising,JenisFising " & _
     ", dr.Qty,FORMAT(Harga,0) Harga, format( sum( dr.qty*harga),0)  `Total`  "
        query2 = " select DATE_FORMAT(TanggalRetur,'%d-%m-%Y') `Tgl Retur`,mp.KdFising `No Fising`,TypeFising,JenisFising " & _
         ", dr.Qty,FORMAT(Harga,0) Harga, sum( dr.qty*harga) `Total`  "

        queryFrom += "  from trheaderpo pb  " & _
        "  join trheaderreturbeli hr on hr.No_PO = pb.no_po join trdetailreturbeli dr on dr.KdRetur=hr.KdRetur join MsFising mp on mp.KDFising=dr.KDBarang  "
        queryFrom += "  where  DATE_FORMAT(TanggalRetur,'%Y-%m-%d') >='" & tg1 & "' and DATE_FORMAT(TanggalRetur,'%Y-%m-%d') <='" & tg2 & "'"
        queryFrom += "  and StatusRetur <> 0 and TipeReturBeli=2  "
        queryFrom += "  group by `Tgl Retur`,mp.KdFising,TypeFising,JenisFising,dr.Qty,Harga "
        ...
        Try
            tglMulai = tg1
            tglAkhir = tg2
            dropview("viewCetakLapRBFising")
            createview(query2, "viewCetakLapRBFsing")
        ...
        End Try
    End Sub
    Private Sub FormLapReturBeliFising_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        flagLaporan = "lap_retur_beli_fising"
        CRLaporan.Show()
    End Sub
