...
Public Class FormPurchasePaymentBahanMentahManagamen
    ...
    Private Sub setData()
        Try
            ...
            Dim reader = execute_reader(" Select MB.KdBahanMentah,NamaBahanMentah,BahanMentahSubKategori, " & _
                                    " Harga, Qty,Disc,StatusBarang,NamaBahanMentahTipe " & _
                                    " from TrPurchasePaymentDetail payment " & _
                                    " Join MsBahanMentah MB On payment.KdBarang = MB.KdBahanMentah " & _
            ...
            Do While reader.Read
                ...
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(2).Value = reader(7)
                ...
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(5).Value = reader(4)
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(6).Value = reader(5)
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(7).Value = Subtotal
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(8).Value = reader(6)
            Loop
            reader.Close()
    ...
    Function SavePaymentDetail(ByVal flag As String)
        ...
        For i As Integer = 0 To gridBahanMentah.RowCount - 1
            ...
            Dim Qty = Val(gridBahanMentah.Rows.Item(i).Cells(5).Value)
            Dim KdBahanMentah = gridBahanMentah.Rows.Item(i).Cells(0).Value
            Dim StatusBahanMentah = gridBahanMentah.Rows.Item(i).Cells(8).Value
            ...
            Try
                If PK = "" Then
                    sql = " insert into  trPurchasePayment ( KdPurchasePayment, " & _
                          ...
                          " '" & txtNote.Text & "','" & kdKaryawan & "','" & cmbBayar.Text & "',1)"
        ...
        Try
            ...
            If gridBahanMentah.Rows.Count <> 0 Then
                For i As Integer = 0 To (gridBahanMentah.Rows.Count - 1)
                    Dim total = gridBahanMentah.Rows.Item(i).Cells(7).Value.ToString.Replace(".", "").Replace(",", "")
        ...
        Try
            ...
            If data_carier(0) <> "" Then
                ...
                Dim reader = execute_reader(" Select MB.KdBahanMentah,NamaBahanMentah,BahanMentahSubKategori, " & _
                                            ...
                                            " where KdBarang = pb.KdBarang " & _
                                            " And No_PO = trheaderpb.No_PO " & _
                                            " Group By No_PO,KdBarang ),'Normal') StatusBarang,NamaBahanMentahTipe " & _
                                            " from trdetailpb pb " & _
                                            " Join trheaderpb On pb.No_PB = trheaderpb.No_PB " & _
                                            " Join MsBahanMentah MB On pb.KdBarang = MB.KdBahanMentah " & _
                                            " Join MsBahanMentahTipe On MsBahanMentahTipe.kdBahanMentahTipe = MB.kdBahanMentahTipe " & _
                                            " Join MsBahanMentahSubKategori On MsBahanMentahSubKategori.KdBahanMentahSubKategori = MB.KdBahanMentahSubKategori " & _
                                            " where No_PO = '" & cmbPO.Text & "' " & _
                                            " And TipePB = 1 " & _
                                            " Group by MB.KdBahanMentah,trheaderpb.No_PO,Harga " & _
                                            " order by NamaBahanMentah asc ")
    ...
    Private Sub cmbpb_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbPO.SelectedIndexChanged
        If cmbPO.SelectedIndex <> 0 Then
            ...
            Do While reader.Read
                ...
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(4).Value = FormatNumber(reader(3), 0)
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(5).Value = reader(4)
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(6).Value = reader(9)
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(7).Value = Subtotal
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells(8).Value = reader(5)
                idxpb += 1
            Loop
            reader.Close()
