Public Class FormTrPenerimaanBarangM
    ...
    Function getRealQty(ByVal kdBarang As String)
    ...
    Private Sub setData()
        Try
            ...
            Do While reader.Read
                ...
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(2).Value = reader(5)
                ...
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(6).Value = reader(6)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(7).Value = FormatNumber(Subtotal, 0)
    ...
    Private Sub cmbPO_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbPO.SelectedIndexChanged
        If cmbPO.SelectedIndex <> 0 Then
            ...
            reader = Nothing
            sql = " select dpo.kdbarang,namabarang,merk,subkategori, " & _
                  ...
                  " join msbarang b on b.kdbarang=dpo.kdbarang " & _
                  " Join MsMerk On MsMerk.kdMerk = b.kdMerk " & _
                  " Join MsSubkategori On MsSubkategori.KdSub = b.KdSub " & _
                  " where no_PO='" & cmbPO.Text & "' " & _
                  " AND dpo.jumlah - ifnull(( Select sum(Qty) " & _
                  " from trdetailpb " & _
                  " Join trheaderpb On trdetailpb.No_PB = trheaderpb.No_PB " & _
                  " where KdBarang = dpo.KdBarang " & _
                  " And No_PO = dpo.No_PO " & _
                  " Group By No_PO,KdBarang ),0) <> 0 "
            reader = execute_reader(sql)
            Dim harga As Double = 0
            gridBarang.Rows.Clear()
            Do While reader.Read
                gridBarang.Rows.Add()
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(0).Value = Trim(reader(0))
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(1).Value = Trim(reader(1))
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(2).Value = Trim(reader(2))
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(3).Value = Trim(reader(3))
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(4).Value = FormatNumber(reader(4), 0)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(5).Value = Val(reader(5))
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(6).Value = 0
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(7).Value = FormatNumber(reader(6), 0)
    ...
    Private Sub gridBarang_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridBarang.CellEndEdit
        Try
            If gridBarang.CurrentRow.Cells(4).Value <> "" Then
                gridBarang.CurrentRow.Cells(4).Value = gridBarang.CurrentRow.Cells(4).Value.Replace(".", "").Replace(",", "")
            End If
            Dim harga = Val(gridBarang.CurrentRow.Cells(4).Value)
            Dim qty = Val(gridBarang.CurrentRow.Cells(5).Value)
            Dim Disc = Val(gridBarang.CurrentRow.Cells(6).Value)
            Dim RealPenerimaan = Val(gridBarang.CurrentRow.Cells(8).Value)
            ...
            If qty <= 0 Or qty > RealPenerimaan Then
                ...
                qty = 1
                gridBarang.CurrentRow.Cells(5).Value = 1
                gridBarang.CurrentRow.Cells(5).Selected = True
            ElseIf Disc < 0 Or IsNumeric(gridBarang.CurrentRow.Cells(6).Value) = False Then
    ...
    Function SavePBDetail(ByVal flag As String)
        ...
        For i As Integer = 0 To gridBarang.RowCount - 1
            ...
            Dim realQty = getRealQty(KdBarang)
            Dim HargaDisc = (Val(harga)) * ((100 - Val(disc)) / 100)

            If flag = 1 Then
                StockBarang(Qty, OP, HargaDisc, KdBarang, Attribute, Trim(txtID.Text), "Form Penerimaan Barang")
            End If
    ...
    Function save(ByVal flag As String)
        ...
        Else
            ...
            Try
                ...
                trans.Commit()
                deleteTabelTemp()
                msgInfo("Data berhasil disimpan. Silakan cetak Penerimaan Barang")
                Me.Close()
            Catch ex As Exception
                trans.Rollback()
                MsgBox(ex, MsgBoxStyle.Critical)
