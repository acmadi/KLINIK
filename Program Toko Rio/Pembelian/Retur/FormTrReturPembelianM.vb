...
Public Class FormTrReturPembelianM
    ...
    Private Sub setData()
        Try
            ...
            Dim reader = execute_reader(" Select MB.KdBarang,NamaBarang,Subkategori, " & _
                                    ...
                                    " And KdBarang = dr.KdBarang " & _
                                    " And No_PO = '" & cmbPO.Text & "' ),0) as QtyFaktur,StatusBarang,Merk " & _
                                    " from TrdetailReturbeli dr " & _
                                    " Join MsBarang MB On dr.KdBarang = MB.KdBarang " & _
            ...
            Do While reader.Read
                ...
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(5).Value = reader(6)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(6).Value = reader(5)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(7).Value = reader(4)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(8).Value = Subtotal
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(9).Value = reader(7)
            Loop
            reader.Close()
    ...
    Function SaveReturDetail(ByVal flag As String)
        ...
        For i As Integer = 0 To gridBarang.RowCount - 1
            ...
            Dim Qty = Val(gridBarang.Rows.Item(i).Cells(7).Value)
            ...
            Dim Stok = gridBarang.Rows.Item(i).Cells(5).Value
            Dim statusBarang = gridBarang.Rows.Item(i).Cells(9).Value
    ...
    Function save(ByVal flag As String)
        ...
        ElseIf cmbStatusRetur.SelectedIndex = 0 Then
            msgInfo("Status Penerimaan barang  harus dipilih")
        ...
        Else
            ...
            For i As Integer = 0 To (gridBarang.Rows.Count - 1)
                If gridBarang.Rows.Item(i).Cells(7).Value <> 0 Then
                    checkQty += 1
                    If gridBarang.Rows.Item(i).Cells(9).Value = "- Klik disini -" Then
                        msgInfo("Klik status barang.")
                        gridBarang.Rows.Item(i).Cells(9).Selected = True
                        Return True
                        Exit Function
                    End If
                Else
    ...
    Private Sub HitungTotal()
        Try
            ...
            If gridBarang.Rows.Count <> 0 Then
                For i As Integer = 0 To (gridBarang.Rows.Count - 1)
                    Dim total = gridBarang.Rows.Item(i).Cells(8).Value.ToString.Replace(".", "").Replace(",", "")
    ...
    Private Sub gridBarang_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridBarang.CellClick
        If gridBarang.CurrentRow.Cells(9).Selected = True Then
            sub_form = New FormBrowseStatusRetur
            sub_form.showDialog(FormMain)
            If data_carier(0) <> "" Then
                gridBarang.CurrentRow.Cells(9).Value = data_carier(0)
                clear_variable_array()
            End If
        End If
    End Sub
    ...
    Private Sub gridBarang_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridBarang.CellEndEdit
        Try
            ...
            Dim harga = Val(gridBarang.CurrentRow.Cells(4).Value.Replace(".", "").Replace(",", ""))
            Dim qty = Val(gridBarang.CurrentRow.Cells(7).Value)
            Dim disc = Val(gridBarang.CurrentRow.Cells(6).Value)
            Dim stok = Val(gridBarang.CurrentRow.Cells(5).Value)
            Dim qtyStock = general_check_stock(BarangID, qty, "Stock")
            If IsNumeric(qty) = False Then
                MsgBox("Jumlah retur harus berupa angka.", MsgBoxStyle.Information, "Validation Error")
                qty = stok
                gridBarang.CurrentRow.Cells(7).Value = stok
                gridBarang.CurrentRow.Cells(7).Selected = True
            ElseIf qty > stok Then
                MsgBox("Jumlah retur melebihi barang yang diterima", MsgBoxStyle.Information, "Validation Error")
                qty = stok
                gridBarang.CurrentRow.Cells(7).Value = stok
                gridBarang.CurrentRow.Cells(7).Selected = True
                ...
                '    harga = 1
                '    gridBarang.CurrentRow.Cells(4).Value = 1
                '    gridBarang.CurrentRow.Cells(4).Selected = True
            ElseIf qty > qtyStock Then
                MsgBox("Jumlah retur melebihi stock barang", MsgBoxStyle.Information, "Validation Error")
                qty = qtyStock
                gridBarang.CurrentRow.Cells(7).Value = qtyStock
                gridBarang.CurrentRow.Cells(7).Selected = True
            ...
            Else
                ...
                gridBarang.CurrentRow.Cells(4).Value = TempHarga
            End If
            ' MsgBox(FormatNumber((Val(harga) * Val(qty)), 0))
            gridBarang.CurrentRow.Cells(8).Value = FormatNumber((Val(harga) * Val(qty)) * ((100 - Val(disc)) / 100), 0)
    ...
    Private Sub cmbfaktur_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbPO.SelectedIndexChanged
        If cmbPO.SelectedIndex <> 0 Then
            Dim reader = execute_reader(" Select MB.KdBarang,NamaBarang,Subkategori, " & _
                                                ...
                                                " Group By No_PO,KdBarang ),0) Qty, " & _
                                                " Disc,MT.KdSupplier, Nama, Merk " & _
                                                " from Trdetailpb dp " & _
                                                " Join trheaderpb pb On dp.no_pb = pb.no_pb " & _
                                                " Join MsBarang MB On dp.KdBarang = MB.KdBarang " & _
                                                " Join MsMerk On MsMerk.kdMerk = MB.kdMerk " & _
                                                " Join MsSubkategori On MsSubkategori.KdSub = MB.KdSub " & _
            ...
            Do While reader.Read
                ...
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(0).Value = reader(0)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(1).Value = reader(1)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(2).Value = reader("Merk")
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(3).Value = reader(2)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(4).Value = FormatNumber(reader(3), 0) 'harga
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(5).Value = reader(4) 'qty
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(6).Value = reader("Disc")
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(7).Value = 0
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(8).Value = 0
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells(9).Value = "- Klik disini -"
                idxfaktur += 1
            Loop
            reader.Close()
