...
Public Class FormSalesPayment
    ...
    Private Sub viewAllData(ByVal cr As String, ByVal opt As String)
        ...
        sql = " select KdSalesPayment 'No. Pembayaran',DATE_FORMAT(TanggalSalesPayment,'%d %M %Y') Tanggal, " & _
              " Pembayaran.kdFaktur 'No. Faktur',NamaLengkap 'Nama User', NamaSales 'Nama Sales', " & _
              " NamaToko 'Nama Toko', mt.Daerah, format(Pembayaran.TotalSalesPayment,0) 'Total Bayar', " & _
              " CASE WHEN StatusSalesPayment = 0 THEN 'New' " & _
              " WHEN StatusSalesPayment = 1 THEN 'Confirm' End 'Status Bayar', " & _
        ...
        If opt <> "" Then
            ...
            If col = "TanggalSalesPayment" Then
                sql &= " And DATE_FORMAT(TanggalSalesPayment, '%Y/%m/%d') >= '" & dtpFrom.Value.ToString("yyyy/MM/dd") & "' " & _
                                   " and DATE_FORMAT(TanggalSalesPayment, '%Y/%m/%d') <= '" & dtpTo.Value.ToString("yyyy/MM/dd") & "' " & _
                                   " and KdSalesPayment <>'SP00000000'"
            Else
                sql &= "  And " & col & "  like '%" & cr & "%'  and KdSalesPayment <>'SP00000000'"
            End If
        Else
            sql &= "  And KdSalesPayment <>'FK00000000'"
        End If
        sql &= " Order By StatusSalesPayment asc,Pembayaran.no_increment Desc "
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        Try
            open_subpage("FormSalesPaymentManagamen")
            viewAllData("", "")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            If (DataGridView1.RowCount) Then
                If DataGridView1.CurrentRow.Cells("Status Bayar").Value = "New" Then
                    Dim selected_cell = DataGridView1.CurrentRow.Cells(0).Value
                    If MsgBox("Anda yakin ingin menghapus kode payment " & selected_cell & "?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmation") = MsgBoxResult.Yes Then
                        execute_update("delete from trsalespaymentdetail where  " & PK & " = '" & selected_cell & "' ")
                        execute_update("delete from " & tab & " where  " & PK & " = '" & selected_cell & "' And StatusSalesPayment = 0 ")
